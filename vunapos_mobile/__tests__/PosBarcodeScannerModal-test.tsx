...
jest.mock("expo-camera", () => ({
  CameraView: (props: object) =>
    require("react").createElement("CameraView", props),
...
describe("PosBarcodeScannerModal", () => {
  ...
  it("requests access instead of opening a camera preview when permission is denied", async () => {
    ...
    const screen = await render(
      <PosBarcodeScannerModal onClose={jest.fn()} onScan={jest.fn()} visible />,
    );
  ...
  it("locks duplicate camera callbacks until an unknown barcode is acknowledged", async () => {
    ...
    const screen = await render(
      <PosBarcodeScannerModal onClose={jest.fn()} onScan={onScan} visible />,
    );
    ...
    await act(async () => {
      screen
        .getByTestId("barcode-camera")
        .props.onBarcodeScanned({ data: "next-code", type: "ean13" });
    });
  ...
  it("closes the scanner after a barcode is accepted", async () => {
    ...
    const screen = await render(
      <PosBarcodeScannerModal
        onClose={onClose}
        onScan={jest.fn().mockResolvedValue(null)}
        visible
      />,
    );
    ...
    await act(async () => {
      screen
        .getByTestId("barcode-camera")
        .props.onBarcodeScanned({ data: "0123456789", type: "ean13" });
    });
    expect(onClose).toHaveBeenCalledTimes(1);
  });

  it("explains an unreadable camera result and lets the cashier try again", async () => {
    mockUseCameraPermissions.mockReturnValue([
      { granted: true },
      mockRequestPermission,
    ]);
    ...
    await act(async () => {
      screen
        .getByTestId("barcode-camera")
        .props.onBarcodeScanned({ data: " ", type: "ean13" });
    });
    expect(
      screen.getByText(
