...
describe("PosCheckoutScreen", () => {
  ...
  it("validates a loyalty redemption through the cart preview and recalculates the payment", async () => {
    submit.mockResolvedValue({
      doctype: "Sales Invoice",
      name: "SINV-LOYALTY-001",
    });
    const previewLoyalty = jest.fn().mockImplementation((points: number) =>
      Promise.resolve({
        items: [],
    ...
    await fireEvent.changeText(
      screen.getByLabelText("Loyalty points to redeem"),
      "10",
    );
    ...
    await waitFor(() =>
      expect(screen.getByText("Applied: 10 points · KES 20.00")).toBeTruthy(),
    );
    await waitFor(() =>
      expect(screen.getByLabelText("Cash amount").props.value).toBe("96.00"),
    );
    ...
    await waitFor(() =>
      expect(screen.queryByLabelText("Remove loyalty redemption")).toBeNull(),
    );
    await waitFor(() =>
      expect(screen.getByLabelText("Cash amount").props.value).toBe("116.00"),
    );
  });
  ...
  it("shows a Tax ID field only for a walk-in customer and submits its trimmed value", async () => {
    ...
    expect(screen.getByLabelText("Customer Tax ID").props.placeholder).toBe(
      "P012345678X",
    );
    await fireEvent.changeText(
  ...
  it("allows a permitted delivery charge to be applied through the parent cart", async () => {
    ...
    mockUsePosBootstrap.mockReturnValue({
      data: {
        payment_modes: [{ default: true, mode_of_payment: "Cash" }],
        pos_profile: {
          allow_delivery_charge_change: true,
          allow_delivery_charges: true,
          delivery_charge_item: "DELIVERY",
          name: "POS-001",
        },
      },
      error: null,
    ...
    await waitFor(() =>
      expect(applyDeliveryCharge).toHaveBeenCalledWith("DELIVERY", 25),
    );
  });
  ...
  it("sends the selected credit sale due date and clears old submission errors", async () => {
    ...
    await fireEvent(
      screen.getByLabelText("Enable credit sale"),
      "valueChange",
      true,
    );
    await fireEvent.press(screen.getByLabelText("Complete sale"));
  ...
  it("uses the POS Profile credit-sale default and provides the SPA-style switch", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByLabelText("Enable credit sale").props.value).toBe(
        true,
      ),
    );
  ...
  it("submits a split allocation across configured manual payment modes", async () => {
    ...
    await waitFor(() =>
      expect(submit).toHaveBeenCalledWith(
        expect.objectContaining({
          payments: [
            { amount: 16, mode_of_payment: "Cash" },
            { amount: 100, mode_of_payment: "M-Pesa" },
          ],
        }),
      ),
    );
  ...
  it("requires and submits a transaction reference for a configured bank payment mode", async () => {
    ...
    expect(
      screen.getByLabelText("Bank transfer transaction reference"),
    ).toBeTruthy();
    expect(
      screen.getByText(
        "A transaction reference is required for Bank transfer.",
      ),
    ).toBeTruthy();
    expect(
  ...
  it("blocks gateway checkout until the server confirms the gateway payment", async () => {
    ...
    expect(
      screen.getByText(
        "Verify the selected gateway payment to continue. Checkout unlocks after confirmation.",
      ),
    ).toBeTruthy();
    expect(
      screen.getByLabelText("Complete sale").props.accessibilityState.disabled,
    ...
    await fireEvent.press(
      screen.getByLabelText("Check gateway payment status"),
    );
  ...
  it("unlocks checkout immediately when the matching gateway link is confirmed in realtime", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByLabelText("Complete sale").props.accessibilityState
          .disabled,
      ).toBe(true),
    );
  ...
  it("searches for and attaches an exact matching C2B payment before enabling checkout", async () => {
    ...
    await fireEvent.press(
      screen.getByLabelText("Search incoming C2B payments"),
    );
  ...
  it("disables completion when an electronic payment exceeds the invoice total", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText("Only cash can exceed the total and return change."),
      ).toBeTruthy(),
    );
    expect(
      screen.getByLabelText("Complete sale").props.accessibilityState.disabled,
  ...
  it("blocks a Sales Order advance above the order total", async () => {
    ...
    expect(
      screen.getByText("An advance cannot exceed the Sales Order total."),
    ).toBeTruthy();
    expect(
      screen.getByLabelText("Submit sales order").props.accessibilityState
        .disabled,
    ).toBe(true);
  });
});
