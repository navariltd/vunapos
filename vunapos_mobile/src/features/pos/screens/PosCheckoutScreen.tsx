...
export function PosCheckoutScreen({
  ...
  const [paymentReferences, setPaymentReferences] = useState<
    Record<string, { referenceDate: string; referenceNo: string }>
  >({});
  const [gatewayLinks, setGatewayLinks] = useState<
    Record<string, PosGatewayPaymentLink | undefined>
  >({});
  ...
  const [isDeliveryDatePickerVisible, setIsDeliveryDatePickerVisible] =
    useState(false);
  const [referenceDateMode, setReferenceDateMode] = useState<string | null>(
    null,
  );
  ...
  const updateGatewayPaymentFromRealtime = useCallback(
    (payment: PosGatewayPaymentLink) => {
      setGatewayLinks((current) => {
        ...
        return {
          ...current,
          [payment.mode_of_payment]: { ...currentLink, ...payment },
        };
      });
    },
    [],
  ...
  const manualModes = (bootstrap.data?.payment_modes ?? [])
    ...
    .map((mode) => ({
      ...
      requires_reference:
        ...
        profilePaymentModes.find(
          (profileMode) => profileMode.mode_of_payment === mode.mode_of_payment,
        )?.requires_reference,
    }));
  const gatewayModes = (bootstrap.data?.payment_modes ?? []).filter((mode) =>
    Boolean(mode.payment_gateway),
  );
  const paymentModes = [...manualModes, ...gatewayModes];
  const invoiceTotal = isInvoice
    ? (preview.data?.totals.rounded_total ??
      preview.data?.totals.grand_total ??
      0)
    : subtotal;
  ...
  const loyaltyConversionFactor = Math.max(
    customerLoyalty.data?.conversion_factor ?? 0,
    0,
  );
  const maximumLoyaltyPoints =
    loyaltyConversionFactor > 0
      ? Math.min(
          ...
          Math.floor(invoiceTotal / loyaltyConversionFactor),
        )
      : 0;
  const loyaltyInputPoints = /^\d+$/.test(loyaltyInput)
    ? Number(loyaltyInput)
    : null;
  const loyaltyInputError = Boolean(
    loyaltyInput &&
    (!loyaltyInputPoints || loyaltyInputPoints > maximumLoyaltyPoints),
  );
  ...
  const deliveryChargeEnabled = Boolean(
    profile?.allow_delivery_charges && deliveryChargeItem,
  );
  const deliveryChargeCanChange =
    profile?.allow_delivery_charge_change !== false;
  const deliveryChargeCartItem = items.find(
    (item) => item.item_code === deliveryChargeItem,
  );
  const displayedDeliveryChargeAmount =
  ...
  const allocationInputs = buildPaymentInputs(
    paymentModes,
    paymentAmounts,
    precision,
    paymentReferences,
  ...
  const paymentModeKey = paymentModes
    .map((mode) => mode.mode_of_payment)
    .join("|");
  const hasNonCashOverpayment = allocation.nonCashMinor > totalMinor;
  const missingReferenceMode = paymentInputs.find(
    (payment) =>
      manualModes.find(
        (mode) => mode.mode_of_payment === payment.mode_of_payment,
      )?.requires_reference && !payment.reference_no,
  )?.mode_of_payment;
  const hasMissingPaymentReference = Boolean(missingReferenceMode);
  const hasUnverifiedGatewayPayment = allocationInputs.some(
    (payment) =>
      gatewayModes.some(
        (mode) => mode.mode_of_payment === payment.mode_of_payment,
      ) && !isSuccessfulGatewayPayment(gatewayLinks[payment.mode_of_payment]),
  );
  const allowsSalesOrderAdvancePayments = Boolean(
    !isInvoice && profile?.allow_sales_order_payments,
  );
  const hasSalesOrderAdvanceOverpayment =
    allowsSalesOrderAdvancePayments && allocation.allocatedMinor > totalMinor;
  const canUseCredit = Boolean(isInvoice && profile?.allow_credit_sales);
  const canSubmitPayment = Boolean(
    isInvoice
      ? (totalMinor === 0 || paymentModes.length) &&
          (isCreditSale
            ? !allocation.hasInvalidAmount && !hasNonCashOverpayment
            : canCompletePaymentAllocation(
                allocation,
                totalMinor,
                Boolean(profile?.allow_partial_payment),
              )) &&
  ...
  const paymentBalanceLabel = isPaymentOverpaid
    ? "Change"
    : isCreditSale ||
        (allocation.remainingMinor > 0 && profile?.allow_partial_payment)
      ? "Outstanding"
      : "Remaining";
  ...
  const isReadyToSubmit = Boolean(
    ...
    (isInvoice
      ? isCreditSale
        ? dueDate >= postingDate && canSubmitPayment
        : canSubmitPayment
      : deliveryDate >= today() && canSubmitPayment),
  );
  const paidAmount = allocation.allocatedMinor / currencyScale(precision);
  const checkoutBalanceAmount = Math.abs(
    allocation.remainingMinor / currencyScale(precision),
  );
  ...
  useEffect(() => {
    ...
    setIsCreditSale(
      Boolean(
        isInvoice &&
        profile.allow_credit_sales &&
        profile.default_sale_type === "Credit Sale",
      ),
    );
  ...
  useEffect(() => {
    if (!isInvoice || isCreditSale || !manualModes.length || totalMinor <= 0)
      return;
    ...
    setPaymentAmounts((current) => {
      ...
      const defaultMode =
        manualModes.find((mode) => mode.default) || manualModes[0];
      const currentMinor =
        parsePaymentAmount(
          current[defaultMode.mode_of_payment] || "",
          precision,
        ) || 0;
      return {
        ...current,
        [defaultMode.mode_of_payment]: minorUnitsToInput(
          Math.max(currentMinor + totalMinor - previousTotalMinor, 0),
          precision,
        ),
      };
    });
  ...
  async function refreshGatewayPayment() {
    ...
    if (nextLink)
      setGatewayLinks((current) => ({ ...current, [modeOfPayment]: nextLink }));
  }
  ...
  async function attachC2BGatewayPayment(payment: PosC2BGatewayPayment) {
    ...
    const modeOfPayment = activeGatewayMode.mode_of_payment;
    const amountMinor =
      parsePaymentAmount(paymentAmounts[modeOfPayment] || "", precision) || 0;
    if (
      !amountMinor ||
      totalToMinorUnits(payment.amount, precision) !== amountMinor
    )
      return;
    const idempotencyKey =
      gatewayIdempotencyKeys.current[modeOfPayment] ||
  ...
  function setPaymentReferenceNo(mode: string, referenceNo: string) {
    setPaymentReferences((current) => ({
      ...current,
      [mode]: {
        referenceDate: current[mode]?.referenceDate || today(),
        referenceNo,
      },
    }));
  }
  ...
  function setSaleType(nextCreditSale: boolean) {
    ...
    setPaymentAmounts(
      nextCreditSale
        ? Object.fromEntries(
            paymentModes.map((mode) => [mode.mode_of_payment, ""]),
          )
        : createInitialPaymentAmounts(manualModes, totalMinor, precision),
    );
    setGatewayLinks({});
    setActiveGatewayMode(null);
    initializedPaymentKey.current = nextCreditSale
      ? null
      : `${paymentModeKey}:${totalMinor}`;
  ...
  async function applyDeliveryCharge() {
    ...
    if (amountMinor === null) {
      setDeliveryChargeError(
        `Enter a valid delivery charge using up to ${precision} decimal places.`,
      );
      return;
    }
    setDeliveryChargeError(null);
    setIsApplyingDeliveryCharge(true);
    const applied = await onApplyDeliveryCharge(
      deliveryChargeItem,
  ...
  async function applyLoyaltyPoints(points: number) {
    ...
    try {
      ...
      const validatedInvoiceTotal =
        loyaltyPreview.totals.rounded_total ??
        loyaltyPreview.totals.grand_total ??
        0;
      const validatedPayableMinor = totalToMinorUnits(
        Math.max(
          validatedInvoiceTotal - (loyaltyPreview.loyalty_amount ?? 0),
          0,
        ),
        precision,
      );
      ...
      setPaymentAmounts(
        isCreditSale
          ...
          : createInitialPaymentAmounts(
              manualModes,
              validatedPayableMinor,
              precision,
            ),
      );
      setGatewayLinks({});
      setActiveGatewayMode(null);
      initializedPaymentKey.current = isCreditSale
        ? null
        : `${paymentModeKey}:${validatedPayableMinor}`;
    } catch (error) {
      setLoyaltyError(
  ...
  function requestSubmit() {
    ...
    if (isCreditSale && dueDate < postingDate) {
      setValidationError(
        "The credit sale due date cannot be before the posting date.",
      );
      return;
    }
    ...
    if (
      ...
      setValidationError(
        "Enter valid advance-payment amounts using the configured currency precision.",
      );
      return;
    }
    if (!isInvoice && hasSalesOrderAdvanceOverpayment) {
      setValidationError(
        "Sales Order advance payments cannot exceed the order total.",
      );
      return;
    }
    if (hasMissingPaymentReference) {
      setValidationError(
        `Enter the transaction reference for ${missingReferenceMode}.`,
      );
      return;
    }
    if (hasUnverifiedGatewayPayment) {
      setValidationError(
        "Verify the selected gateway payment before completing this sale.",
      );
      return;
    }
    if (isInvoice) {
      ...
      if (allocation.hasInvalidAmount) {
        setValidationError(
          "Enter valid payment amounts using the configured currency precision.",
        );
        return;
      }
      if (hasNonCashOverpayment) {
        setValidationError(
          "Electronic payments cannot exceed the invoice total.",
        );
        return;
      }
      if (
      ...
      if (!isLoyaltySelectionValid) {
        setValidationError(
          "The available loyalty balance changed. Apply a valid number of points again.",
        );
        return;
      }
    }
  ...
  if (bootstrap.error || (isInvoice && preview.error)) {
    return (
      <View style={styles.state}>
        ...
        >
          <Text style={styles.backToCartLabel}>Back to cart</Text>
        </Pressable>
      </View>
    );
  }
...
function PaymentSummary({ label, value }: { label: string; value: string }) {
  return (
    <View style={styles.paymentSummary}>
      ...
      <Text numberOfLines={1} style={styles.paymentSummaryValue}>
        {value}
      </Text>
    </View>
  );
