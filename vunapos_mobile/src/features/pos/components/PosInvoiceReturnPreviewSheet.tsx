...
function quantityFor(
  item: PosInvoiceReturnPreviewItem,
...
export function PosInvoiceReturnPreviewSheet({
  currency,
  currencyPrecision = 2,
  invoiceName,
  onComplete,
  ...
  posProfile,
  visible,
}: PosInvoiceReturnPreviewSheetProps) {
  ...
  const preview = useInvoiceReturnPreview({
    enabled: visible,
    invoiceName,
    posProfile,
  });
  ...
  const [createdReturn, setCreatedReturn] = useState<PosInvoiceReturn | null>(
    null,
  );
  const displayCurrency = preview.data?.currency || currency;
  const returnableItems =
    preview.data?.items.filter((item) => item.returnable_qty > 0) ?? [];
  const selectedItems = returnableItems.flatMap(
    (item): SelectedReturnItem[] => {
      const qty = quantityFor(item, quantities);
      return qty > 0 ? [{ qty, row_name: item.row_name }] : [];
    },
  );
  const estimatedCredit = returnableItems.reduce(
    (total, item) => total + quantityFor(item, quantities) * item.rate,
    0,
  );
  ...
  function updateQuantity(item: PosInvoiceReturnPreviewItem, value: string) {
    ...
    const nextValue =
      Number.isFinite(quantity) && quantity > item.returnable_qty
        ? String(item.returnable_qty)
        : value;
  ...
  function toggleItem(item: PosInvoiceReturnPreviewItem) {
    ...
    setQuantities((current) => ({
      ...current,
      [item.row_name]: isSelected ? "" : String(item.returnable_qty),
    }));
    setValidationError(null);
  }
  ...
  function returnAllAvailableItems() {
    setSelectedRows(
      Object.fromEntries(returnableItems.map((item) => [item.row_name, true])),
    );
    setQuantities(
      Object.fromEntries(
  ...
  async function createReturn() {
    ...
    setCreatedReturn({
      ...
      grand_total:
        result.invoice.totals.rounded_total ||
        result.invoice.totals.grand_total ||
        0,
    ...
    if (
      selectedItems.some((selected) => {
        const item = returnableItems.find(
          (candidate) => candidate.row_name === selected.row_name,
        );
        return (
          !item ||
          !Number.isFinite(selected.qty) ||
          selected.qty > item.returnable_qty
        );
      })
    ) {
      setValidationError(
        "One or more quantities are no longer available. Review the selected items.",
      );
      return;
    }
  ...
  return (
    ...
    >
      <View style={styles.modalRoot}>
        <Pressable
          accessibilityLabel="Dismiss return items"
            ...
            <View style={styles.header}>
              <View style={styles.heading}>
                ...
                <Text style={styles.subtitle}>
                  Create a credit note against {invoiceName}.
                </Text>
              </View>
              <Pressable
                accessibilityLabel="Close return items"
                disabled={isSubmitting}
                onPress={onDismiss}
            ...
            {createdReturn ? (
              <View style={styles.successState}>
                <Text style={styles.successTitle}>
                  {createdReturn.name} created
                </Text>
                <Text style={styles.successText}>
                  The credit note has been submitted successfully.
                </Text>
                <Pressable
                  accessibilityLabel="View created credit note"
                  onPress={() => onComplete(createdReturn)}
              ...
              <View style={styles.state}>
                <Text style={styles.stateText}>Preparing return…</Text>
              </View>
            ) : null}
            {!createdReturn && preview.error ? (
              <View style={styles.state}>
            ...
            {!createdReturn && preview.data ? (
              ...
              >
                      ...
                      >
                        ...
                        <View style={styles.quantities}>
                          <Text style={styles.quantity}>
                            Sold {formatQuantity(item.sold_qty, item.uom)}
                          </Text>
                          <Text style={styles.quantity}>
                            Returned{" "}
                            {formatQuantity(item.returned_qty, item.uom)}
                          </Text>
                          <Text style={styles.available}>
                            Available{" "}
                            {formatQuantity(item.returnable_qty, item.uom)}
                          </Text>
                        </View>
                        <Pressable
                          accessibilityLabel={`${selected ? "Remove" : "Add"} ${item.item_name} ${selected ? "from" : "to"} return`}
                          disabled={isSubmitting}
                          onPress={() => toggleItem(item)}
                          style={[
                            styles.itemAction,
                            selected && styles.itemActionSelected,
                          ]}
                        >
                          <Text
                            style={[
                              styles.itemActionLabel,
                              selected && styles.itemActionLabelSelected,
                            ]}
                          >
                            {selected ? "Remove item" : "Add to return"}
                          </Text>
                        </Pressable>
                        {selected ? (
                ...
                {returnableItems.length ? (
                  <>
                    <Pressable
                      accessibilityLabel="Return all available items"
                      disabled={isSubmitting}
                      onPress={returnAllAvailableItems}
                      style={styles.returnAllButton}
                    >
                      <Text style={styles.returnAllButtonLabel}>
                    ...
                    <View style={styles.summary}>
                      ...
                      <Text style={styles.summaryAmount}>
                        {formatCurrency(estimatedCredit, displayCurrency)}
                      </Text>
                    </View>
                    ...
                    <Pressable
                      accessibilityLabel="Create return credit note"
                      disabled={
                        !selectedItems.length || !reason.trim() || isSubmitting
                      }
...
function createStyles(palette: AppPalette) {
  ...
  itemActionLabelSelected: { color: palette.onPrimary },
  itemActionSelected: {
  ...
  itemSelected: { borderColor: palette.primary },
