...
const statusOptions: {
  label: string;
  value: PosInvoiceHistoryFilters["status"];
}[] = [
...
const saleTypeOptions: {
  label: string;
  value: PosInvoiceHistoryFilters["saleType"];
}[] = [
...
type PosInvoiceFiltersSheetProps = {
  ...
  onApply: () => void;
  onChange: <Key extends keyof PosInvoiceHistoryFilters>(
    field: Key,
...
function DatePickerField({
  ...
  return (
    ...
    >
      <Text
        numberOfLines={1}
        style={[styles.datePickerValue, !value && styles.datePickerPlaceholder]}
      >
...
export function PosInvoiceFiltersSheet({
  ...
  const [datePickerField, setDatePickerField] =
    useState<DateFilterField | null>(null);
  const [selection, setSelection] = useState<FilterSelection | null>(null);
  const paymentModeOptions: {
    label: string;
    value: PosInvoiceHistoryFilters["paymentMode"];
  }[] = [
  ...
  const selectedDate = datePickerField
    ? parseFilterDate(filters[datePickerField])
    : new Date();
        ...
        >
          <View accessibilityViewIsModal style={styles.sheet}>
            ...
            <View style={styles.sheetHeader}>
              <View>
                ...
                <Text style={styles.subtitle}>
                  Choose criteria, then apply them to sales history.
                </Text>
              </View>
              <Pressable
            ...
            {datePickerField ? (
              <DateTimePicker
                maximumDate={
                  datePickerField === "fromDate" && filters.toDate
                    ? parseFilterDate(filters.toDate)
                    : new Date()
                }
                minimumDate={
                  datePickerField === "toDate" && filters.fromDate
                    ? parseFilterDate(filters.fromDate)
                    : undefined
                }
                mode="date"
