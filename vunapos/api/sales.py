...
from vunapos.services.checkout_queue_service import (
	cancel_queued_invoice as cancel_queued_invoice_service,
)
...
@frappe.whitelist(methods=["POST"])
def cancel_queued_invoice(pos_profile: str, invoice_name: str):
	try:
		return success(cancel_queued_invoice_service(pos_profile, invoice_name))
	except Exception as exc:
		return _failure_from_exception(exc)
