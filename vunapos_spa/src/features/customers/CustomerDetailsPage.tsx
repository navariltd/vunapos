...
type Props = {
  customer: string;
  posProfile?: string;
  onStartSale: (customer: CustomerDetailsDTO["customer"]) => void;
};
...
export function CustomerDetailsPage({
  ...
  if (response.data) {
    try {
      details = unwrapVunaResponse<CustomerDetailsDTO>(response.data);
    } catch (error) {
      parseError =
    ...
    return (
      <section className="flex flex-1 items-center justify-center p-6">
        <div className="max-w-md text-center">
          <p className="text-sm text-error">{error || "Customer not found"}</p>
          <Button
            className="mt-3"
            variant="ghost"
            onClick={() => navigateToPosPage("Customers")}
          >
            Back to customers
          </Button>
        </div>
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="flex gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-on-surface-variant">
                {row.customer} · {row.customer_group || "Uncategorized"}
              </p>
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            <Button onClick={() => onStartSale(row)}>
              <ShoppingCart className="mr-2 size-4" />
              Start new sale
            </Button>
            <Button
              variant="ghost"
              onClick={() => navigateToCustomerPayment(row.customer)}
            >
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <Summary
            ...
            value={money(details.balance, row.currency)}
          />
          <Summary
            ...
            sub={details.loyalty?.tier || details.loyalty?.program}
          />
          <Summary
            label="Customer type"
            value={row.customer_type || "-"}
            sub={row.territory || undefined}
          />
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          <div className="rounded-lg border border-outline-variant p-4">
            ...
            <div className="mt-3 space-y-2 text-sm">
              ...
              <p>{row.email_id || contact?.email_id || "No email address"}</p>
              {row.tax_id ? <p>Tax ID: {row.tax_id}</p> : null}
            </div>
          </div>
          <div className="rounded-lg border border-outline-variant p-4">
            <h3 className="flex items-center gap-2 font-semibold">
            ...
            <div className="mt-3 text-sm text-on-surface-variant">
              {address ? (
                ...
                <p>No permitted primary address available.</p>
              )}
            </div>
          </div>
        </div>
              ...
              <tbody>
                {details.invoices.length ? (
                  details.invoices.map((invoice) => (
                    ...
                    >
                      <td className="px-4 py-3 font-medium">{invoice.name}</td>
                      <td>{formatDate(invoice.posting_date)}</td>
                      <td>
                        <span
                          className={`rounded-full px-2 py-1 text-xs font-medium ${statusStyle[invoice.status] || ""}`}
                        >
                          {invoice.status}
                        </span>
                      </td>
                      <td>{money(invoice.grand_total, invoice.currency)}</td>
                      <td>{money(invoice.paid_amount, invoice.currency)}</td>
                      <td>
                        {money(invoice.outstanding_amount, invoice.currency)}
                      </td>
                      <td>
                        <a
                          className="text-primary hover:underline"
                          href={`/app/${invoice.doctype.toLowerCase().replaceAll(" ", "-")}/${encodeURIComponent(invoice.name)}`}
                          target="_blank"
                          rel="noreferrer"
                        >
                          View
        ...
        <div className="rounded-lg border border-outline-variant">
          ...
          {details.payments.length ? (
            details.payments.map((payment) => (
              <div
                key={payment.name}
                className="grid gap-2 border-t border-outline-variant px-4 py-3 text-sm first:border-t-0 sm:grid-cols-[1.3fr_1fr_1fr_1fr]"
              >
                <div>
                ...
                <p>{payment.mode_of_payment || "Unspecified"}</p>
                <p>{money(payment.received_amount, row.currency)}</p>
                <p className="text-on-surface-variant">
                  Unallocated {money(payment.unallocated_amount, row.currency)}
                </p>
              </div>
            ))
          ) : (
