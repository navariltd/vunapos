    ...
    <section className="h-full overflow-y-auto p-4 sm:p-6">
      <div className="mx-auto max-w-3xl">
        <div className="flex items-start gap-4 border-b border-outline-variant pb-5">
          ...
          <div className="min-w-0 flex-1">
            <h2 className="truncate text-xl font-semibold text-on-surface">
              {bootstrap?.current_user || "Cashier"}
            </h2>
            <p className="mt-1 text-sm text-on-surface-variant">
              VunaPOS cashier profile
            </p>
          </div>
          <a
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          ...
          <ProfileField
            label="Invoice Mode"
            value={bootstrap?.invoice_mode || "Sales Invoice"}
          />
          <ProfileField
            label="Default Sale Type"
            value={bootstrap?.default_sale_type || "Cash Sale"}
          />
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs text-on-surface-variant">
                Choose how VunaPOS should display for this user.
              </p>
            </div>
...
function formatDefaultCustomer(customer: BootstrapData["default_customer"]) {
  ...
  return (
    (customer as CustomerDTO).customer_name ||
    (customer as CustomerDTO).customer
  );
}
