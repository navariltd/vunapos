...
export function InvoicesPage({
  ...
  let error = call.error?.message || "";
  try {
    if (call.data) history = unwrapVunaResponse<History>(call.data);
  } catch (err) {
    error =
      err instanceof Error ? err.message : "Unable to load invoice history";
  }
  ...
  return (
    <section
      onClick={openDetails}
      className="min-h-0 flex-1 overflow-y-auto border-t border-outline-variant bg-surface p-4 pb-[84px] lg:pb-4"
    >
          ...
          <>
            <div className="grid gap-3 rounded-lg border border-outline-variant p-4 sm:grid-cols-2 lg:grid-cols-4">
              <input
                ...
                onChange={(event) => update("invoice", event.target.value)}
              />
              <input
                className={fieldClass}
              ...
              <select
                ...
                onChange={(event) => update("payment_mode", event.target.value)}
              >
              ...
              <select
                ...
                onChange={(event) => update("sale_type", event.target.value)}
              >
              ...
              <label className="flex items-center gap-2 text-sm">
                <input
                  type="checkbox"
                  checked={filters.current_shift === "1"}
                  onChange={(event) =>
                    update("current_shift", event.target.checked ? "1" : "0")
                  }
                />
                Current shift only
              </label>
            ...
            {history ? (
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                <Summary
                  ...
                  <Summary
                    label="Gross sales"
                    value={money(history.summary.gross_sales, currency)}
                  />
                  <Summary
                    label="Returns"
                    value={money(history.summary.returns, currency)}
                  />
                  <Summary
                    label="Outstanding"
                    value={money(history.summary.outstanding, currency)}
                  />
                  <Summary
                    label="Credit sales"
                    value={money(history.summary.credit_sales, currency)}
                  />
                  <Summary
                    label="Credit outstanding"
                    value={money(history.summary.credit_outstanding, currency)}
                  />
                </div>
                          ...
                          >
                            <td className="px-3 py-3">
                              ...
                              {row.vunapos_credit_sale ? (
                                <span className="mt-1 block w-fit rounded-full bg-tertiary-container px-2 py-0.5 text-xs font-medium text-on-tertiary-container">
                                  Credit Sale
                                </span>
                              ) : null}
                            </td>
                            ...
                            <td>
                              {row.vunapos_session_cashier || "-"}
                              <span className="block text-xs text-on-surface-variant">
                                {row.vunapos_opening_entry || "No shift"}
                              </span>
                            </td>
                            <td>{row.total_qty}</td>
                            <td>
                              {row.payments.length
                                ? row.payments.map((payment) => (
                                    <PaymentLine
                                      key={`${payment.mode_of_payment}-${payment.transaction_reference || payment.ke_payment_request || payment.amount}`}
                                      payment={payment}
                            ...
                            <td className="font-medium">
                              {money(row.grand_total, row.currency || currency)}
                            </td>
                            <td>
                              {money(
                ...
                <div className="flex justify-end gap-2">
                  <Button
                    variant="ghost"
                    disabled={!start}
                    onClick={() => setStart(Math.max(0, start - 50))}
                  >
                    Previous
                  </Button>
                  <Button
...
function Status({ value }: { value: string }) {
  return (
    <span
      className={`rounded-full px-2 py-1 text-xs font-medium ${value === "Paid" ? "bg-secondary-container text-on-secondary-container" : value === "Cancelled" || value === "Overdue" ? "bg-error-container text-on-error-container" : "bg-surface-container-high text-on-surface"}`}
    >
      {value}
    </span>
  );
