...
type CartItemRowProps = {
  ...
  onUpdateSerialAllocations: (
    rowName: string,
    allocations: SerialAllocationDTO[],
  ) => Promise<void>;
  isOnline: boolean;
  warehouse?: string;
};
...
export function CartItemRow({
  ...
  const quantity =
    quantityDraft?.rowName === item.row_name &&
    quantityDraft.sourceQty === item.qty
      ? quantityDraft.value
      : String(item.qty);
  ...
  const isBatchTracked = Boolean(
    item.has_batch_no || item.batch_no || item.batch_allocations?.length,
  );
  const isSerialTracked = Boolean(
    item.has_serial_no || item.serial_and_batch_bundle,
  );
  ...
  return (
    <article
      className={`overflow-hidden rounded-md border bg-surface-container-low transition-colors ${expanded ? "border-primary shadow-sm" : "border-outline-variant"}`}
    >
      <div className={expanded ? "bg-surface-container p-3" : "p-3"}>
        <div className="flex items-start justify-between gap-3">
          <button
            ...
            onClick={() => {
              if (!item.is_free_item) onToggle(item.row_name);
            }}
          ...
          >
            <span className="min-w-0">
              <span className="flex items-center gap-2">
                ...
                {item.is_free_item ? (
                  <span className="rounded-full bg-primary-container px-2 py-0.5 text-[10px] font-semibold uppercase text-on-primary-container">
                    Free item
                  </span>
                ) : null}
            ...
            <ChevronDown
              className={`mt-1 size-4 shrink-0 text-on-surface-variant transition-transform ${expanded ? "rotate-180" : ""}`}
            />
          </button>
          <button
            type="button"
            className="flex size-8 shrink-0 items-center justify-center rounded-md text-error hover:bg-error-container disabled:opacity-50"
        ...
        <div className="mt-3 flex items-center justify-between gap-3">
          <div className="flex items-center overflow-hidden rounded-md border border-outline-variant bg-surface">
            <button
              type="button"
              disabled={itemDisabled || item.qty <= 1}
              className="flex h-10 w-10 items-center justify-center hover:bg-surface-container-low disabled:opacity-50"
              onClick={() => {
                setQuantity(String(item.qty - 1), item.qty - 1);
                void onUpdateQty(item.row_name, item.qty - 1).catch(() =>
                  setQuantity(String(item.qty)),
                );
              }}
      ...
      {expanded && !item.is_free_item ? (
        <div
          id={`cart-item-details-${item.row_name}`}
          className="space-y-4 border-t border-primary bg-surface-container-low p-4"
        >
          {description ? (
          ...
          {item.pricing_override ? (
            <div className="mt-3 rounded-md border border-tertiary bg-tertiary-container px-3 py-2 text-xs text-on-tertiary-container">
              <span className="font-semibold">Manual price override:</span>{" "}
              {pricingOverrideLabel(item.pricing_override, currency)}
              {item.pricing_override_by
                ? ` · ${item.pricing_override_by}`
                : " · current cashier"}
            </div>
          ) : item.pricing_rules ? (
            <div className="mt-3 rounded-md bg-secondary-container px-3 py-2 text-xs text-on-secondary-container">
              Promotion or pricing rule applied: {item.pricing_rules}
            </div>
          ) : null}
          ...
          <label className="flex items-center justify-between gap-5 rounded-md border border-outline-variant bg-surface p-3">
            ...
            <select
              className="h-[34px] min-w-0 flex-1 rounded-md border border-outline-variant bg-surface px-2 text-[13px]"
              disabled={disabled || (item.uoms?.length || 0) < 2}
              value={item.uom || item.stock_uom || ""}
              onChange={(event) => {
                const selected = item.uoms?.find(
                  (row) => row.uom === event.target.value,
                );
                if (selected)
                  void onUpdateUom(
                    item.row_name,
            ...
            >
              {(item.uoms?.length
                ...
                <option key={row.uom} value={row.uom}>
                  {row.uom} ({row.conversion_factor} {item.stock_uom})
                </option>
              ))}
            </select>
          </label>
          ...
          <ItemNoteEditor
            key={item.item_note || "empty-note"}
            disabled={Boolean(disabled)}
            item={item}
            onSave={(note) => onUpdateNote(item.row_name, note)}
          />
          ...
          {isSerialTracked ? (
            <div className="overflow-hidden rounded-md border border-outline-variant bg-surface">
              <button
                type="button"
                className="flex w-full items-center justify-between p-3 text-left hover:bg-surface-container"
                onClick={() => setSerialExpanded((value) => !value)}
              >
                <span>
                  <span className="block text-xs font-medium uppercase tracking-wide text-on-surface-variant">
                    Serial numbers
                  </span>
                ...
                <ChevronDown
                  className={`size-4 transition-transform ${serialExpanded ? "rotate-180" : ""}`}
                />
              </button>
              {serialExpanded ? (
                <SerialAllocationEditor
                  ...
                  item={item}
                  onSave={(rows) =>
                    onUpdateSerialAllocations(item.row_name, rows)
                  }
                />
              ) : null}
            </div>
          ...
          {isBatchTracked && !isSerialTracked ? (
            <div className="overflow-hidden rounded-md border border-outline-variant bg-surface">
              <button
                type="button"
                className="flex w-full items-center justify-between gap-3 p-3 text-left hover:bg-surface-container"
                onClick={() => setBatchExpanded((value) => !value)}
              ...
              >
                ...
                <ChevronDown
                  className={`size-4 text-on-surface-variant transition-transform ${batchExpanded ? "rotate-180" : ""}`}
                />
              </button>
              {batchExpanded ? (
                <BatchAllocationEditor
                  ...
                  onSave={(allocations) =>
                    onUpdateBatchAllocations(item.row_name, allocations)
                  }
                />
              ) : null}
            </div>
    ...
    <div className="overflow-hidden rounded-md border border-outline-variant bg-surface">
      ...
      >
        <span>
          ...
          <span className="mt-1 block max-w-72 truncate text-xs text-on-surface-variant">
            {note.trim() || "No note added"}
          </span>
        </span>
        <ChevronDown
          className={`size-4 shrink-0 text-on-surface-variant transition-transform ${expanded ? "rotate-180" : ""}`}
        />
      </button>
      {expanded ? (
        <label className="block border-t border-outline-variant p-3">
          <textarea
            className="min-h-20 w-full resize-y rounded-md border border-outline-variant bg-surface px-3 py-2 text-sm outline-none focus:border-primary"
            disabled={disabled}
            maxLength={500}
          ...
          />
          <span className="mt-1 flex justify-between text-xs text-on-surface-variant">
            <span>
              {error ? (
...
function SerialAllocationEditor({
  ...
  data: ItemBatchesDTO | null;
  disabled: boolean;
  item: InvoiceItemDTO;
  onSave: (rows: SerialAllocationDTO[]) => Promise<void>;
}) {
  const [query, setQuery] = useState("");
  const [selected, setSelected] = useState(
    () => new Set((item.serial_allocations || []).map((row) => row.serial_no)),
  );
  ...
  const filtered = serials.filter((row) =>
    row.serial_no.toLowerCase().includes(query.toLowerCase()),
  );
  ...
  const scan = () => {
    const exact = serials.find(
      (row) => row.serial_no.toLowerCase() === query.trim().toLowerCase(),
    );
    if (exact) {
      toggle(exact.serial_no);
      setQuery("");
    }
  };
  return (
    <div className="border-t border-outline-variant p-3">
      <div className="flex gap-2">
        <input
          className="h-9 min-w-0 flex-1 rounded-md border border-outline-variant bg-surface px-2 text-sm"
      ...
      <div className="mt-3 max-h-56 space-y-2 overflow-y-auto">
        {filtered.map((row) => (
          <label
            key={row.serial_no}
            className="flex items-center gap-2 rounded-md border border-outline-variant bg-surface p-2 text-sm"
          >
            <input
              type="checkbox"
              checked={selected.has(row.serial_no)}
              disabled={
                disabled ||
                saving ||
                (!selected.has(row.serial_no) && selected.size >= required)
              }
              onChange={() => toggle(row.serial_no)}
            />
            <span className="min-w-0 flex-1 truncate">{row.serial_no}</span>
            {row.batch_no ? (
              <span className="text-xs text-on-surface-variant">
                {row.batch_no}
              </span>
...
function PricingEditor({
  ...
  const baseRate = taxInclusive
    ? numericRate / (1 + inclusiveTaxRate / 100)
    : numericRate;
  ...
  const changeDiscountPercentage = (rawValue: string) => {
    ...
    setDiscountAmount(
      rawValue !== "" && Number.isFinite(value)
        ? String(Math.round(priceListRate * value) / 100)
        : "",
    );
  };
  ...
  const changeDiscountAmount = (rawValue: string) => {
    ...
    setDiscountPercentage(
      rawValue !== "" && Number.isFinite(value) && priceListRate > 0
        ? String(Math.round((value / priceListRate) * 10000) / 100)
        : "",
    );
  };
    ...
    <div className="rounded-md border border-outline-variant bg-surface p-3">
      <div className={taxRate > 0 ? "grid grid-cols-2 gap-3" : ""}>
        <PriceInput
          label={
            mixedTaxTreatment
              ? "Selling rate (partly incl. tax)"
              : taxInclusive
                ? "Selling rate (incl. tax)"
                : taxRate > 0
                  ? "Selling rate (excl. tax)"
                  : "Selling rate"
          }
          value={rate}
          readOnly={!allowRateChange || disabled}
          onChange={setRate}
          onApply={() => apply("rate", rate)}
          onReset={item.pricing_override ? () => void reset() : undefined}
          resetDisabled={disabled || saving}
        />
        {taxRate > 0 ? (
          <ReadOnlyPrice
            label={
              mixedTaxTreatment || !taxInclusive
                ? "Rate (incl. tax)"
                : "Base rate"
            }
            value={
              mixedTaxTreatment || !taxInclusive ? inclusiveRate : baseRate
            }
            currency={currency}
          />
        ) : null}
      ...
      {taxRate > 0 ? (
        <div className="mt-3 grid grid-cols-2 gap-3">
          <ReadOnlyValue label="Tax rate" value={`${taxRate}%`} />
          <ReadOnlyPrice
            label="Tax per unit"
            value={taxPerUnit}
            currency={currency}
      ...
      <div className="mt-3 grid grid-cols-2 gap-3">
        <PriceInput
          ...
          readOnly={!allowDiscountChange || disabled}
          max={100}
          onChange={changeDiscountPercentage}
          onApply={() => apply("discount_percentage", discountPercentage)}
        />
        <PriceInput
          ...
          onApply={() => apply("discount_amount", discountAmount)}
        />
      </div>
...
function ReadOnlyPrice({
  ...
  return (
    <ReadOnlyValue label={label} value={formatCurrency(value, currency)} />
  );
}
...
function ReadOnlyValue({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <div className="mt-1 flex h-[34px] items-center justify-end rounded-md border border-outline-variant bg-surface-container px-2 text-[13px] text-on-surface-variant">
        {value}
      </div>
    </div>
  );
...
function PriceInput({
  ...
  return (
    <div>
      ...
      <input
        aria-label={label}
        className={`mt-1 h-[34px] w-full rounded-md border border-outline-variant px-2 text-right text-[13px] outline-none ${readOnly ? "bg-surface-container text-on-surface-variant" : "bg-surface focus:border-primary"}`}
        readOnly={readOnly}
        inputMode="decimal"
        min="0"
        max={max}
        ...
        onBlur={() => {
          if (!readOnly && value !== "") onApply();
        }}
        onKeyDown={(event) => {
          if (event.key === "Enter" && !readOnly) event.currentTarget.blur();
        }}
      />
    </div>
  );
...
function BatchAllocationEditor({
  ...
  const [amounts, setAmounts] = useState<Record<string, string>>(() =>
    Object.fromEntries(
      (item.batch_allocations || []).map((row) => [
        row.batch_no,
        String(row.qty),
      ]),
    ),
  ...
  const allocated = parsed.reduce(
    (sum, batch) => sum + (Number.isFinite(batch.qty) ? batch.qty : 0),
    0,
  );
  const remaining = requiredQty - allocated;
  const invalid = parsed.some(
    (batch) => batch.qty < 0 || batch.qty > Number(batch.available_qty || 0),
  );
  ...
  const save = async (rows = parsed) => {
    ...
    try {
      ...
      setActionError(
        saveError instanceof Error
          ? saveError.message
          : "Failed to save the batch allocation",
      );
    } finally {
      setSaving(false);
    }
  ...
  const selectAutomatic = async () => {
    ...
    try {
      ...
      setActionError(
        saveError instanceof Error
          ? saveError.message
          : "Failed to clear the batch allocation",
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="border-t border-outline-variant p-3">
      <div className="flex items-start justify-between gap-3">
        <div>
          {batchData?.verified_at ? (
            <p className="mt-1 text-xs text-on-surface-variant">
              Verified {new Date(batchData.verified_at).toLocaleString()}
            </p>
          ) : null}
        </div>
        <Button
      ...
      {error ? (
        <div className="mt-3 flex gap-2 text-sm text-error">
        ...
        <>
          <div className="mt-3 grid grid-cols-3 gap-2 rounded-md bg-surface p-2 text-center text-xs">
            <div>
              ...
              <p className="font-semibold">{requiredQty}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p
                className={
                  remaining < 0 ? "font-semibold text-error" : "font-semibold"
                }
              >
                {remaining}
              </p>
          ...
          <div className="mt-3 max-h-60 space-y-2 overflow-y-auto pr-1">
            {batches.map((batch) => (
              <label
                key={batch.batch_no}
                className="grid grid-cols-[minmax(0,1fr)_6rem] items-center gap-3 rounded-md border border-outline-variant bg-surface p-2"
              >
                <span className="min-w-0">
                ...
                <input
                  aria-label={`${batch.batch_no} allocation`}
                  className="h-9 w-full rounded-md border border-outline-variant bg-surface px-2 text-right text-sm"
                  disabled={disabled || saving}
                  inputMode="decimal"
                  min="0"
                  max={Number(batch.available_qty || 0)}
                  step="any"
                  type="number"
                  ...
                  onChange={(event) =>
                    setAmounts((current) => ({
                      ...current,
                      [batch.batch_no]: event.target.value,
                    }))
                  }
                />
              </label>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <Button
              ...
              size="sm"
              className="gap-1"
              disabled={disabled || saving || !batches.length}
              onClick={autoAllocate}
            >
              <WandSparkles className="size-3.5" /> Auto allocate FEFO
            </Button>
            <Button
              variant="ghost"
              size="sm"
              disabled={disabled || saving || !item.batch_allocations?.length}
              onClick={() => void selectAutomatic()}
            >
              Use automatic
            </Button>
            <Button
