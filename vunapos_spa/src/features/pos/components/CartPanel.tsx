...
export function CartPanel({
  ...
  const showRoundedTotal = Boolean(
    roundedTotal && Math.abs(roundedTotal - grandTotal) > 0.0001,
  );
  ...
  return (
    <aside
      className={cn(
        "flex min-h-0 flex-col border-t border-outline-variant bg-surface p-4 xl:border-l xl:border-t-0",
        className,
      )}
    >
      <div className="shrink-0">
        ...
        {allowPriceListSwitching && allowedPriceLists.length ? (
          <select
            ...
            onChange={(event) =>
              onSelectPriceList(
                event.target.value === defaultPriceList
                  ? undefined
          ...
          >
            {defaultPriceList ? (
              <option value={defaultPriceList}>
                {defaultPriceList} (Default)
              </option>
            ) : null}
            {allowedPriceLists
              .filter((priceList) => priceList.name !== defaultPriceList)
              .map((priceList) => (
        ...
        {items?.length ? (
              ...
              <CartItemRow
                ...
                onToggle={() =>
                  setExpandedRow((current) =>
                    current === rowIdentity ? null : rowIdentity,
                  )
                }
          ...
          <div className="rounded-md border border-dashed border-outline-variant bg-surface-container-low p-6 text-center">
            ...
            <p className="mt-1 text-xs text-on-surface-variant">
              Select an item to start a sale.
            </p>
          </div>
        )}
      </div>
        ...
        <div className="space-y-1.5 text-xs">
          <div className="flex justify-between">
            ...
            <span className="font-medium">
              {formatCurrency(invoice?.totals?.net_total, currency)}
            </span>
          </div>
          {taxes.map((tax, index) => (
            <div
          ...
          <div className="flex justify-between">
            ...
            <span className="font-medium">
              {formatCurrency(
                invoice?.totals?.total_taxes_and_charges,
                currency,
              )}
            </span>
          </div>
          <div className="flex justify-between text-sm">
            ...
            <span className="font-semibold text-on-surface">
              {formatCurrency(invoice?.totals?.grand_total, currency)}
            </span>
          </div>
          {showRoundedTotal ? (
            <div className="flex justify-between text-sm">
        ...
        <div className="mt-5 grid grid-cols-2 gap-2">
          ...
          <div className="col-span-2">
            <Button
              className="w-full"
              disabled={
                isMutating || !items?.length || invoice?.docstatus !== 0
              }
              onClick={onCheckout}
            >
