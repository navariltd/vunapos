...
export function ItemTaxPrice({
  ...
  const originalDisplayRate =
    Number(item.rate || 0) > 0
      ? effectiveRate * (originalRate / Number(item.rate))
      : originalRate;
  const hasPricingDiscount = Boolean(
    item.pricing_rule && originalDisplayRate > effectiveRate,
  );
  const productPromotion =
    item.pricing_rule?.kind === "product"
      ? item.pricing_rule.free_items
          ?.map((freeItem) =>
            `Get ${freeItem.qty} ${freeItem.uom || ""} ${freeItem.item_name || freeItem.item_code} free`.replace(
              /\s+/g,
              " ",
            ),
          )
  ...
  const price = (
    <div
      className={alignment}
      title={item.pricing_rule?.pricing_rules.join(", ") || tax?.template}
    >
      {hasPricingDiscount ? (
        <p className="text-xs text-on-surface-variant line-through">
          {formatCurrency(originalDisplayRate, currency)}
        </p>
      ) : null}
      ...
      {hasPricingDiscount ? (
        <p className="text-xs font-medium text-primary">
          {item.pricing_rule?.discount_percentage}% off
        </p>
      ) : null}
      {productPromotion ? (
        <p className="max-w-44 text-xs font-medium text-primary">
          {productPromotion}
        </p>
      ) : null}
    </div>
