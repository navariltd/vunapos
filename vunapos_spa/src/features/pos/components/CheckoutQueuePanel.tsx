...
export function CheckoutQueuePanel({
  ...
  try {
    ...
    loadError =
      error instanceof Error ? error.message : "Unable to load queued sales";
  }

  async function retryInvoice(invoice: string) {
    setActiveInvoice(invoice);
    setActionError("");
    try {
      const response = await retryCall.call({
        pos_profile: posProfile,
      ...
      setActionError(
        error instanceof Error ? error.message : "Unable to retry queued sale",
      );
    } finally {
      setActiveInvoice("");
    }
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-start justify-between gap-3 rounded-lg border border-outline-variant bg-surface-container-low p-4">
        <div>
          ...
          <p className="text-sm text-on-surface-variant">
            Reserved sales awaiting submission or operator review.
          </p>
        </div>
        <Button
          variant="ghost"
          disabled={call.isLoading}
          onClick={() => void call.mutate()}
        >
      ...
      {call.isLoading ? (
                  ...
                  >
                    <td className="px-4 py-3">
                      <a
                        ...
                        href={`/app/sales-invoice/${encodeURIComponent(row.name)}`}
                        target="_blank"
                        rel="noreferrer"
                      >
                        {row.name}
                      ...
                      <span className="block text-xs text-on-surface-variant">
                        {row.queued_at || row.posting_date || ""}
                      </span>
                    </td>
                    <td>
                    ...
                    <td className="font-medium">
                      {money(
                        row.rounded_total || row.grand_total || 0,
                        row.currency || currency,
                      )}
                    </td>
                    <td>
                      <QueueStatus value={row.queue_status} />
                    </td>
                    <td>{row.queue_attempts || 0}</td>
                    ...
                    <td className="px-4">
                      <div className="flex justify-end">
                        <Button
                          size="sm"
                          variant="ghost"
                          disabled={!retryable || Boolean(activeInvoice)}
                          onClick={() => void retryInvoice(row.name)}
                        >
        ...
        <div className="rounded-lg border border-dashed border-outline-variant p-10 text-center">
          <p className="font-medium">The checkout queue is clear</p>
          <p className="mt-1 text-sm text-on-surface-variant">
            There are no reserved sales waiting for submission or review.
          </p>
        </div>
      )}
    </div>
...
function QueueStatus({ value }: { value: QueueRow["queue_status"] }) {
  const tone =
    value === "Failed" || value === "Requires Review"
