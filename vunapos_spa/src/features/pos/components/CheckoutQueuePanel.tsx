...
export function CheckoutQueuePanel({ posProfile, currency }: { posProfile?: string; currency?: string }) {
	...
	async function retryInvoice(invoice: string) {
		...
		setActionError("");
		try {
			const response = await retryCall.call({ pos_profile: posProfile, invoice_name: invoice });
			unwrapVunaResponse(response);
			await call.mutate();
		} catch (error) {
			setActionError(error instanceof Error ? error.message : "Unable to retry queued sale");
		} finally {
			setActiveInvoice("");
		}
	}
	...
	return <div className="flex flex-col gap-4">
		<div className="flex flex-wrap items-start justify-between gap-3 rounded-lg border border-outline-variant bg-surface-container-low p-4">
			<div><h3 className="font-semibold">Checkout queue</h3><p className="text-sm text-on-surface-variant">Reserved sales awaiting submission or operator review.</p></div>
